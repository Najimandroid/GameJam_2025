...
UI_TextButton::UI_TextButton(sf::Vector2f position, sf::Vector2f size)
    : m_shape(size),
...
UI_TextButton::UI_TextButton(sf::Vector2f position, sf::Vector2f size, const std::string& text)
    : m_shape(size),
...
void UI_TextButton::set_callback(std::function<void()> callback)
{
    m_callback = callback;
}

// Center the text inside the button
void UI_TextButton::center_text()
{
...
void UI_TextButton::set_text(const std::string& text)
{
    m_text.setString(text);
}

// Set the text size (character size)
void UI_TextButton::set_text_size(unsigned int size)
{
    m_text.setCharacterSize(size);
}

// Set the text fill color
void UI_TextButton::set_text_color(const sf::Color& color)
{
    m_text.setFillColor(color);
}

// Set the button's fill color
void UI_TextButton::set_fill_color(const sf::Color& color)
{
    m_shape.setFillColor(color);
}

// Set the button's outline thickness and color
void UI_TextButton::set_outline(float thickness, const sf::Color& color)
{
...
void UI_TextButton::update(float deltaTime)
{
    center_text();
}

// Render the button (shape and text)
void UI_TextButton::render(sf::RenderWindow& window)
{
    if (m_isVisible)
    {
        window.draw(m_shape);
...
void UI_TextButton::handle_event(const sf::Event& event, const sf::RenderWindow& window)
{
...
sf::FloatRect UI_TextButton::get_bounds() const
{
